...
/// Week-grid lesson picker (Schedule look) for attaching a payment to a journal lesson.
class LessonCalendarPickerPage extends StatefulWidget {
  const LessonCalendarPickerPage({
  ...
  /// When true, only unpaid, billable (non-free) lessons are shown.
  final bool unpaidOnly;

  @override
  State<LessonCalendarPickerPage> createState() =>
      _LessonCalendarPickerPageState();
}

class _LessonCalendarPickerPageState extends State<LessonCalendarPickerPage> {
  ...
  bool _matchesStudentFilter(Lesson lesson) {
  ...
  bool _isSelectable(Lesson lesson) {
    if (lesson.status == 'cancelled') {
      return false;
    }
    if (!_matchesStudentFilter(lesson)) {
      return false;
    }
    if (widget.unpaidOnly) {
      ...
      if (lesson.resolvedPaymentStatus != PaymentStatus.unpaid) {
        return false;
      }
    }
    return true;
  ...
  List<Lesson> _lessonsForDay(DateTime day) {
    ...
    return _weekLessons.where((Lesson lesson) {
      ...
      return dayKey == key && _isSelectable(lesson);
    }).toList();
  }
  ...
  Future<void> _loadWeek() async {
    ...
    try {
      ...
      final List<Lesson> lessons = await widget.lessonService.calendar(
  ...
  void _scrollToWorkHours() {
    ...
    const int focusHour = 8;
    final double offset = (focusHour * _hourHeight) -
        (_gridScrollController.position.viewportDimension / 4);
  ...
  Widget build(BuildContext context) {
    ...
    return CupertinoPageScaffold(
      navigationBar: CupertinoNavigationBar(
        middle: Text(l10n.pickLessonFromCalendar),
  ...
  Widget _buildGrid() {
    ...
    final bool anyVisible = _weekDays.any(
      (DateTime day) => _lessonsForDay(day).isNotEmpty,
    );
      ...
      children: <Widget>[
        if (!anyVisible)
          Padding(
            ...
            child: Text(
              context.l10n.noLessonsThisWeek,
              textAlign: TextAlign.center,
              style: TextStyle(
                fontSize: 13,
                color: CupertinoColors.secondaryLabel.resolveFrom(context),
...
class _PickerDayColumn extends StatelessWidget {
  const _PickerDayColumn({
    required this.day,
